...
current set of namespaces is available in `process.sqreen_namespaces`, so library code
that wants to use continuation-local storage only when it's active should test
for the existence of `process.sqreen_namespaces`.
